riot.tag('item', '<div class="Event-buttonsBox item-{ id }"> <span title="{ period_that_occurs }" class="Event-button Event-dayButton Event--newsfeed js-EventDayButton is-schedule has-tooltip"> { day_of_week } { start_hour } </span> <span onclick="{ schedule }" class="Event-button Event-agendaButton js-EventAgendaButton { is-schedule: scheduled }"> <span class="Event-buttonText js-EventButtonText">{ button_text }</span> <span class="Event-agendedByCount js-agendedByCount has-tooltip" title="{ agended_by.text }">{agended_by.count}</span> </span> </div> <div class="js-EventNewsfeedTransitions panel panel-default shadow-animation"> <div class="Event-content"> <a href="{ base_url + link }" data-push="true"> <div class="Event-overlay"></div> </a> <div class="Event-detailsBox"> <div class="Event-place"> <span class="glyphicon glyphicon-map-marker"></span> <a href="{ base_url + place.link }">{ place.name }</a> </div> </div> <div class="Event-imageBox b-lazy" data-src="{ image }"></div> <div class="caption"> <span class="Event-subCat" if="{ subcategories }"> { subcategories } </span> <h2 class="Event-title"> <a href="{ base_url + link }" data-push="true">{ name }</a> </h2> <span class="description"> { description } </span> <div class="Event-infos"> <span class="Event-infosPrice event.price[:css_attributes] Event-infosItem"> { price } </span> <span if="{ rating }" class="Event-infosRating Event-infosItem"> <span class="Event-infosRatingStar glyphicon glyphicon-star"></span> { rating } </span> <span class="Event-infosFriends pull-right Event-infosItem">   <i class="has-tooltip avatar-icon" title=" friend.first_name agendou o evento">  </i>   </span> </div> </div> </div> </div>', 'onmouseenter="{ mouseEnterEvents }" onmouseleave="{ mouseLeaveEvents }"', function(opts) {
    ...
    this.mouseEnterEvents = function(event) {
      item = this;
    ...
    this.mouseLeaveEvents = function(event) {
      var delay = 4000;
