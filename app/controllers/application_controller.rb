...
class ApplicationController < ActionController::Base
  ...
  def configure_permitted_parameters
	  devise_parameter_sanitizer.permit(:sign_up, keys: [:name, :username, :email, :avatar, :address, :password, :password_confirmation, :persona_ids => []])
    devise_parameter_sanitizer.permit(:account_update, keys: [:name, :username, :email, :avatar, :address, :password, :password_confirmation, :current_password, :city_id, :neighborhood_id])
	end
  ...
  def agended(event, user = current_user)
      if user.nil?
        false
	    elsif user.items_agenda.include?(event)
        true
	    else
        false
	    end
  ...
  def to_url(url)
    ...
    formatted_url = if formatted_url.include?(".com.br")
