	...
	def index
		...
		if params[:key]
			if InviteService.create_user_from_invite(params[:key])
				redirect_to sign_in_path, notice: 'Faça login com a senha enviada por email.' and return
			else
				redirect_to welcome_path, notice: 'Este convite já foi utilizado ou é invalido.' and return
			end
		end
